...
2. 같은 과제의 파일이 여러 버전이면 날짜, 버전, 최종본 안내를 확인하고 가장 최신인 자료를 선택한다.
...
6. 오류 후보는 `페이지/문항`, `현재 문구`, `오류라고 판단한 이유`, `제안 변경 문구`, `영향받는 채점 항목` 형식의 별도 텍스트 또는 Markdown 문서로 정리한다. 출제 의도를 추측해 원본 PDF, 채점표 또는 배포 파일을 직접 수정하지 않는다.
7. 공식 질의가 필요하면 마이스터넷 등 지정된 채널에 제출할 수 있도록 근거와 재현 절차를 준비한다. 협의회 메모의 72시간 답변 원칙은 기대 시간으로만 취급하며, 답변이 없다는 이유로 임의 변경하지 않는다.
8. 공식 배포 파일은 가능한 한 그대로 재사용한다. 바이너리, 이미지, 샘플 데이터, Lambda 코드의 내용을 임의로 대체하지 않는다.
9. 배포된 추가 Markdown이나 바이너리 실행 안내가 문제지 범위를 넘어서는 경우 숨은 요구사항으로 단정하지 말고, 문제지·채점표·공식 정정에서 근거를 찾아 오류 후보로 기록한다.
10. 다른 응시번호 폴더는 패턴 참고용으로만 읽을 수 있다. 대상 과제의 고정 이름, 리전, 계정별 값 또는 네트워크 구성을 다른 과제에서 가져오지 않는다.
...
- 1·2·3과제는 공식 정정 또는 추가 문제로 점수 기준 최대 30% 범위에서 달라질 수 있다는 협의회 내용을 고려한다. 기존 구조를 과도하게 고정하거나 채점 스크립트의 현재 문자열에만 맞추지 말고, 단계별 모듈·변수·자산을 교체할 수 있게 구성한다.
...
- [ ] 문제지 충돌이나 누락 명세를 원본 수정 없이 오류 후보 문서로 정리했다.
- [ ] 과제 단계별로 `.tf` 파일을 분리했다.
