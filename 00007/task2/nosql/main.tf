...
resource "aws_dynamodb_table" "reservation" {
  ...
  attribute { name = "train_id" type = "S" }
  attribute { name = "seat_id" type = "S" }
  attribute { name = "user_id" type = "S" }
  attribute { name = "reserved_at" type = "S" }
...
data "archive_file" "audit" { type = "zip" source_file = "${path.module}/assets/lambda.py" output_path = "${path.module}/audit.zip" }
...
resource "aws_lambda_function" "audit" {
 ...
 runtime = "python3.13"
 timeout = 30
  environment { variables = { AUDIT_TABLE_NAME = aws_dynamodb_table.audit.name } }
}
...
resource "aws_vpc" "app" { cidr_block="10.71.0.0/16"
 enable_dns_hostnames=true
 tags={Name="bigbae-nosql-vpc"} }
resource "aws_internet_gateway" "app" { vpc_id=aws_vpc.app.id }
resource "aws_subnet" "public" { vpc_id=aws_vpc.app.id
 cidr_block="10.71.1.0/24"
 availability_zone=data.aws_availability_zones.available.names[0]
 map_public_ip_on_launch=true }
resource "aws_route_table" "public" { vpc_id=aws_vpc.app.id
...
resource "aws_iam_role_policy" "ec2" {
  ...
  policy=jsonencode({Version="2012-10-17",Statement=[{Effect="Allow",Action=["dynamodb:GetItem","dynamodb:PutItem","dynamodb:UpdateItem","dynamodb:Query"],Resource=[aws_dynamodb_table.reservation.arn,"${aws_dynamodb_table.reservation.arn}/index/*"]}]})
}
resource "aws_iam_instance_profile" "app" { name="bigbae-nosql-app-profile"
...
resource "aws_instance" "app" {
 ...
 iam_instance_profile=aws_iam_instance_profile.app.name
  user_data=templatefile("${path.module}/assets/user-data.sh", { app_b64=base64encode(file("${path.module}/assets/app.py")) })
  tags={Name="bigbae-nosql-app-ec2"}
}
