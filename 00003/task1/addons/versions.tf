...
data "tls_certificate" "eks" { url = data.aws_eks_cluster.main.identity[0].oidc[0].issuer }
data "aws_dynamodb_table" "book" { name = var.table_name }
data "aws_iam_policy_document" "pod_assume" { statement { actions = ["sts:AssumeRole", "sts:TagSession"]
principals { type = "Service"
identifiers = ["pods.eks.amazonaws.com"] } } }
resource "aws_iam_role" "pod" { name = "wsc2026-book-pod-role"
assume_role_policy = data.aws_iam_policy_document.pod_assume.json }
resource "aws_iam_policy" "pod" { name = "wsc2026-book-pod-policy"
policy = jsonencode({ Version = "2012-10-17", Statement = [{ Effect = "Allow", Action = ["dynamodb:PutItem"], Resource = data.aws_dynamodb_table.book.arn }] }) }
resource "aws_iam_role_policy_attachment" "pod" { role = aws_iam_role.pod.name
policy_arn = aws_iam_policy.pod.arn }
resource "aws_eks_pod_identity_association" "book" { cluster_name = var.cluster_name
...
data "aws_iam_policy_document" "lbc_assume" { statement { actions = ["sts:AssumeRoleWithWebIdentity"]
