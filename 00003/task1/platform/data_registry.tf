...
type = "S" }
  global_secondary_index { name = "booking_id-index"
hash_key = "booking_id"
projection_type = "ALL" }
...
resource "aws_ecr_lifecycle_policy" "book" {
  ...
  policy = jsonencode({ rules = [{ rulePriority = 1, description = "keep only v1.0.0", selection = { tagStatus = "tagged", tagPrefixList = ["v"], countType = "imageCountMoreThan", countNumber = 1 }, action = { type = "expire" } }] })
}
