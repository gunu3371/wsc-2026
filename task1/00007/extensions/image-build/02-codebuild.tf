...
resource "aws_iam_role" "image_build" {
  name               = "unicorn-book-image-build-role"
  assume_role_policy = data.aws_iam_policy_document.codebuild_assume.json
}

resource "aws_kms_grant" "image_build" {
  name              = "unicorn-book-image-build-decrypt"
  key_id            = local.input.data_kms_arn
  grantee_principal = aws_iam_role.image_build.arn
  operations        = ["Decrypt"]
}

resource "aws_cloudwatch_log_group" "image_build" {
  name              = "/aws/codebuild/unicorn-book-image-build"
  retention_in_days = 7
}
...
data "aws_iam_policy_document" "image_build" {
  ...
  statement {
    actions   = ["kms:Decrypt"]
    resources = [local.input.data_kms_arn]
  }
  statement {
...
resource "aws_codebuild_project" "image_build" {
  name           = "unicorn-book-image-build"
  description    = "Build the official book binary and push v1.0.0 and latest"
  ...
  source {
    ...
    buildspec = <<-YAML
      ...
      phases:
        pre_build:
          commands:
            - if aws ecr describe-images --repository-name unicorn-concert-app --image-ids imageTag=v1.0.0 >/dev/null 2>&1; then export IMAGE_EXISTS=true; else export IMAGE_EXISTS=false; fi
        ...
        build:
          commands:
            - if [ "$IMAGE_EXISTS" = "false" ]; then docker build --platform linux/amd64 -t "$REPOSITORY_URI:v1.0.0" -t "$REPOSITORY_URI:latest" .; docker push "$REPOSITORY_URI:v1.0.0"; docker push "$REPOSITORY_URI:latest"; else MANIFEST=$(aws ecr batch-get-image --repository-name unicorn-concert-app --image-ids imageTag=v1.0.0 --query 'images[0].imageManifest' --output text); aws ecr put-image --repository-name unicorn-concert-app --image-tag latest --image-manifest "$MANIFEST" || true; fi
        post_build:
          commands:
            - aws ecr start-image-scan --repository-name unicorn-concert-app --image-id imageTag=v1.0.0 >/dev/null 2>&1 || true
    YAML
  }
