...
$digest = & aws @awsArguments ecr describe-images --repository-name "unicorn-concert-app" --image-ids "imageTag=$imageTag" --query "imageDetails[0].imageDigest" --output text
if ([string]::IsNullOrWhiteSpace($digest) -or $digest -eq "None") {
  throw "ECR에서 $imageTag 이미지 digest를 찾지 못했습니다."
}

$repositoryName = "unicorn-concert-app"
...
$detectedFindings = @($findings.PSObject.Properties | Where-Object { [int64]$_.Value -gt 0 })
if ($detectedFindings.Count -gt 0) {
  throw "ECR 이미지 스캔에서 취약점이 발견되었습니다: $findingsJson"
}
Write-Host "ECR 스캔이 완료되었으며 LOW 이상 취약점이 없습니다: $findingsJson"
