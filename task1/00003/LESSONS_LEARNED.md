...
| 해소 | `addons` state가 없어서 Kubernetes가 만든 ALB, Classic ELB, target group, 보안 그룹, IAM 역할/정책이 platform destroy 뒤에 남았다. | root module state 경계는 분리됐지만 addons state 보존·destroy 절차가 지켜지지 않았다. | 각 state를 보존하고 반드시 `delivery → addons → image-build → platform` 순서로 destroy한다. ALB Ingress에 `Project`, `ManagedBy` 태그를 전파한다. |
...
```powershell
# 1. CloudFront 및 Kubernetes/Helm 리소스 제거
...
# 3. 데이터 보호 해제 및 버킷/ECR 강제 정리 기능 반영
