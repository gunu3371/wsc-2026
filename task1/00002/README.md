...
ECR 생성 후 foundation의 `push-image.sh`로 공식 `book` 바이너리 이미지를 push한다. 이어 과제 루트 `terraform.tfvars`의 `config.modules.application.book_image_uri`와 `config.outputs.foundation`을 갱신하고, EKS private endpoint에 접근할 수 있는 환경에서 application을 적용한다.
