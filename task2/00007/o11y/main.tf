...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name = "o11y-public-${count.index + 1}", "kubernetes.io/role/elb" = "1"
  }
}
...
resource "kubernetes_service_account_v1" "otel" {
  metadata {
    name      = "o11y-otel"
    namespace = "monitoring"
  }
}
resource "kubernetes_cluster_role_v1" "otel" {
  metadata {
    name = "o11y-otel"
  }
  rule {
...
resource "kubernetes_cluster_role_binding_v1" "otel" {
  metadata {
    name = "o11y-otel"
  }
  role_ref {
...
resource "kubernetes_daemon_set_v1" "otel" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = "o11y-otel"
        container {
          name  = "otel"
...
resource "kubernetes_config_map_v1" "grafana" {
  metadata {
    name      = "o11y-grafana-provisioning"
    namespace = "monitoring"
  }
  data = {
...
resource "aws_security_group_rule" "alb_app_nodes" {
  type                     = "ingress"
  from_port                = 30080
  to_port                  = 30080
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "alb_grafana_nodes" {
  type                     = "ingress"
  from_port                = 30300
  to_port                  = 30300
  protocol                 = "tcp"
...
resource "aws_lb_target_group_attachment" "app" {
  # Node IDs are discovered only after the managed node group exists. Keep
  # the desired replica count as the stable Terraform instance key.
  count            = 2
  target_group_arn = aws_lb_target_group.app.arn
