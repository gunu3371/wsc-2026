...
locals {
  bucket = "skillsphone-landing-ab-${data.aws_caller_identity.current.account_id}"
}
...
resource "aws_cloudfrontkeyvaluestore_key" "a" {
  key_value_store_arn = aws_cloudfront_key_value_store.ab.arn
  key                 = "version_a"
  value               = "/version-a/index.html"
}
resource "aws_cloudfrontkeyvaluestore_key" "b" {
  key_value_store_arn = aws_cloudfront_key_value_store.ab.arn
  key                 = "version_b"
  ...
  policy = jsonencode({
    Version = "2012-10-17", Statement = [{
      Sid = "CloudFrontRead", Effect = "Allow", Principal = {
        ...
        }, Action = "s3:GetObject", Resource = "${aws_s3_bucket.landing.arn}/*", Condition = {
        StringEquals = {
...
output "distribution_domain" {
  value = aws_cloudfront_distribution.ab.domain_name
}
